...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    h5("Powertrain Type:", style = "font-weight: bold; color: #2c3e50; margin-top: 10px;"),
    checkboxGroupInput(
      "fuel_types",
      NULL,
      choices = list(
    ...
    h5("Vehicle Type:", style = "font-weight: bold; color: #2c3e50; margin-top: 15px;"),
    checkboxGroupInput(
      "vehicle_types",
      NULL,
      choices = list(
        ...
        plots <- lapply(powertrains, function(pt) {
          ...
          p <- p %>%
            layout(
              ...
              yaxis = list(title = "Cumulative Probability", gridcolor = "#e9ecef"),
              plot_bgcolor = "#fafbfc",
      ...
      # --- PDF PLOT ---
        ...
        plots <- lapply(powertrains, function(pt) {
          ...
          for (vt in vehicle_types) {
            ...
            if (nrow(vt_data) > 1) {
              pdf_data <- vt_data %>%
                mutate(pdf = c(diff(quantile) / diff(dvmt), NA)) %>%
          ...
          p <- p %>%
            layout(
              xaxis = list(title = "DVMT", gridcolor = "#e9ecef"),
              yaxis = list(title = "Probability Density", gridcolor = "#e9ecef"),
              plot_bgcolor = "#fafbfc",
              annotations = list(
                x = 0.5, y = 1.05,
                text = paste("<b>", toupper(pt), "</b>"),
