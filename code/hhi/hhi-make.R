...
hhi_powertrain %>%
  ...
  labs(
    x = 'HHI',
    y = 'Powertrain',
    title = 'Vehicle brand level HHI by powertrain',
...
hhi_powertrain <- counts_total %>%
  group_by(GEOID, price_bin, powertrain, make) %>%
  summarise(n = sum(n)) %>%
  ungroup() %>%
  collect() %>%
...
hhi_powertrain %>%
  ...
  labs(
    ...
    x = 'HHI',
    y = 'Powertrain',
    title = 'Vehicle brand level HHI by powertrain and price bin',
    subtitle = 'Higher number indicates greater concentration by brand\nPoint reflects mean across all census tracts, line is 95% lower and upper bounds'
  )

ggsave(
  here("figs", "hhi_make_powertrain_price_bin.png"),
  width = 8,
  height = 5.5,
  dpi = 300
)
...
hhi_powertrain <- counts_total %>%
  ...
  group_by(GEOID, class, powertrain, make) %>%
  summarise(n = sum(n)) %>%
  ungroup() %>%
  collect() %>%
...
hhi_powertrain %>%
  ...
  labs(
    ...
    x = 'HHI',
    y = 'Powertrain',
    title = 'Vehicle brand level HHI by powertrain and urbanicity',
    subtitle = 'Higher number indicates greater concentration by brand\nPoint reflects mean across all census tracts, line is 95% lower and upper bounds'
  )

ggsave(
  here("figs", "hhi_make_powertrain_urbanicity.png"),
  width = 8,
  height = 5.5,
  dpi = 300
)
...
hhi_powertrain <- counts_total %>%
  ...
  group_by(GEOID, inc_bin, powertrain, make) %>%
  summarise(n = sum(n)) %>%
  ungroup() %>%
  collect() %>%
...
hhi_powertrain %>%
  ...
  labs(
    ...
    x = 'HHI',
    y = 'Powertrain',
    title = 'Vehicle brand level HHI by powertrain and income',
    subtitle = 'Higher number indicates greater concentration by brand\nPoint reflects mean across all census tracts, line is 95% lower and upper bounds'
  )

ggsave(
  here("figs", "hhi_make_powertrain_income_bin.png"),
  width = 8,
  height = 5.5,
  dpi = 300
)
...
hhi_powertrain <- counts_total %>%
  ...
  group_by(GEOID, zev, powertrain, make) %>%
  summarise(n = sum(n)) %>%
  ungroup() %>%
  collect() %>%
...
hhi_powertrain %>%
  ...
  labs(
    ...
    x = 'HHI',
    y = 'Powertrain',
    title = 'Vehicle brand level HHI by powertrain and ZEV status',
    subtitle = 'Higher number indicates greater concentration by brand\nPoint reflects mean across all census tracts, line is 95% lower and upper bounds'
  )

ggsave(
  here("figs", "hhi_make_powertrain_zev.png"),
  width = 8,
  height = 5.5,
  dpi = 300
)
...
hhi_powertrain <- counts_total %>%
  ...
  group_by(GEOID, hov, powertrain, make) %>%
  summarise(n = sum(n)) %>%
  ungroup() %>%
  collect() %>%
...
hhi_powertrain %>%
  ...
  labs(
    ...
    x = 'HHI',
    y = 'Powertrain',
    title = 'Vehicle brand level HHI by powertrain and HOV status',
    subtitle = 'Higher number indicates greater concentration by brand\nPoint reflects mean across all census tracts, line is 95% lower and upper bounds'
  )

ggsave(
  here("figs", "hhi_make_powertrain_hov.png"),
  width = 8,
  height = 5.5,
  dpi = 300
)

# HHI powertrain & vehicle_type ----

hhi_powertrain <- counts_total %>%
  group_by(GEOID, vehicle_type, powertrain, make) %>%
  summarise(n = sum(n)) %>%
  ungroup() %>%
  collect() %>%
  group_by(GEOID, vehicle_type, powertrain) %>%
  mutate(total = sum(n)) %>%
  ungroup() %>%
  mutate(
    p = n / total,
...
hhi_powertrain %>%
  ...
  filter(vehicle_type %in% c('Car', 'CUV', 'SUV', 'Pickup')) %>%
  # filter(powertrain != 'Flex-fuel') %>%
  ggplot() +
  geom_pointrange(
    aes(
      x = hhi_mean,
  ...
  labs(
    ...
    x = 'HHI',
    y = 'Powertrain',
    title = 'Vehicle brand level HHI by powertrain and vehicle type',
    subtitle = 'Higher number indicates greater concentration by brand\nPoint reflects mean across all census tracts, line is 95% lower and upper bounds'
  )

ggsave(
  here("figs", "hhi_make_powertrain_vehicle_type.png"),
  width = 8,
  height = 5.5,
  dpi = 300
)
...
hhi %>%
  ...
  labs(
    ...
    x = 'HHI',
    y = 'Powertrain',
    title = 'Vehicle brand level HHI by powertrain and isochrone',
    subtitle = 'Higher number indicates greater concentration by brand\nPoint reflects mean across all census tracts, line is 95% lower and upper bounds'
  )

ggsave(
  here("figs", "hhi_make_powertrain_isochrone.png"),
  width = 8,
  height = 5.5,
  dpi = 300
)
